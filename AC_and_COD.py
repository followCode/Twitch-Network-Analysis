AC_source=set([])
COD_source=set([])
AC=[]
COD=[]
union=[]

isFile = False

node_file = None
edge_file = None
if isFile:
	node_file = open("../../../college/SNA_DATA/demo/ac_and_cod_node.csv", "w")
	node_file.write("Id,Label,category\n")
	edge_file = open("../../../college/SNA_DATA/demo/ac_and_cod_edge.csv", "w")
	edge_file.write("Source,Target,game\n")

streamers = set([])
users = set([])

for i in g.edges:
	if i.game=="AC":
		#AC.append(i)
		AC_source.add(i.source.label)
		union.append(i)
		streamers.add(i.target.label)
	elif i.game=="COD":
		#COD.append(i)
		COD_source.add(i.source.label)
		union.append(i)
		streamers.add(i.target.label)

com_sources= AC_source.intersection(COD_source)

if isFile:
	# Add streamers to node list
	for streamer in streamers:
		node_file.write(""+streamer+","+streamer+",streamer\n")

com_edges=[]

for i in union:
	if i.source.label in com_sources:
		com_edges.append(i)
		if isFile:
			users.add(i.source.label)
			if i.game=="AC":
				edge_file.write(""+i.source.label+","+i.target.label+",AC\n")
			elif i.game=="COD":
				edge_file.write(""+i.source.label+","+i.target.label+",COD\n")

if isFile:
	for user in users:
		node_file.write(""+user+","+user+",user\n")

print("Number of people who watch both: %d"%len(users))
if isFile:
	node_file.close()
	edge_file.close()